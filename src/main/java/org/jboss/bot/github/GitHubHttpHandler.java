...
public final class GitHubHttpHandler extends AbstractJSONServlet {
    ...
    protected void handleRequest(final HttpServletRequest req, final HttpServletResponse resp, final Map<String, String> queryParams, final JSON json) throws IOException {
        ...
        if (event != null) {
            ...
            String reposName = json.get("repository").get("name").asString();
            String owner = json.get("repository").get("owner").get("name").asString();
            if (reposName == null || owner == null) {
                return;
            }
            final String[] channels = bot.getPrefNode().node("github/projects").node(owner).node(reposName).get("channels", "").split("\\s*,\\s*");
